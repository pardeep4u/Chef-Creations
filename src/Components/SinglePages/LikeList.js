      ...
      if (data) {
        ...
        if (data) {
          ...
          */
          ...
          for (let ind = 0; ind < metaDaata.likedposts.length; ind++) {
            const element = metaDaata.likedposts[ind];
            const ndocRef = doc(db, "blogs", `${element}`);
            const generatedID = ndocRef.id;
            const data = await getDoc(ndocRef);
            console.log(data.data());
            dataArray.push({ ...data.data(), generatedID });
            console.log(dataArray);
          }
